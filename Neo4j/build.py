...
g = Graph("http://localhost:7474", username="neo4j", password="123456")  # 连接neo4j，将'xxx'分别改为你的用户名和密码
g.delete_all()  # 清除neo4j中原有的结点等所有信息

with open('D:\\CBR-YG\\CBR\\Neo4j\\case.csv', 'r', encoding='utf-8') as f:
    reader = csv.reader(f)
    for item in reader:
        ...
        #创建关系
        relation1 = Relationship(accident, "桥梁结构", structure)
        relation2 = Relationship(accident, "桥梁材料", material)
        relation3 = Relationship(accident, "桥梁受损部位", part)
        relation4 = Relationship(accident, "事故发生时段", period)
        relation5 = Relationship(accident, "事故发生区域", area)
        relation6 = Relationship(accident, "事故发生地点", location)
        relation7 = Relationship(accident, "事故发生地点等级", level)
        relation8 = Relationship(accident, "事故类型", types)
        relation9 = Relationship(accident, "事故原因", reason)
        relation10 = Relationship(accident, "事故严重程度", degree)
        relation11 = Relationship(accident, "事故发生季节", season)
        #将节点和关系添加到图谱中
